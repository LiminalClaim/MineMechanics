[file name]: redeem.php
[file content begin]
<?php
// redeem.php - Redeem/Withdraw Funds in m² tokens
session_start();
...
// Conversion rate: $1 = 1,000,000 m² tokens
define('M2_PER_DOLLAR', 1000000);
...
// m² balance (1 m² = $1 value, but shown as m² tokens)
$m2_balance = $balance['m2'] ?? 0;
...
$giftCards = [
    'DOORDASH' => ['min' => 15 * M2_PER_DOLLAR, 'max' => 150 * M2_PER_DOLLAR],
    'INSTACART' => ['min' => 25 * M2_PER_DOLLAR, 'max' => 250 * M2_PER_DOLLAR],
    'UBER' => ['min' => 15 * M2_PER_DOLLAR, 'max' => 300 * M2_PER_DOLLAR],
    'UBER_EATS' => ['min' => 15 * M2_PER_DOLLAR, 'max' => 200 * M2_PER_DOLLAR],
    'ADIDAS' => ['min' => 5 * M2_PER_DOLLAR, 'max' => 300 * M2_PER_DOLLAR],
    'STEAM' => ['min' => 5 * M2_PER_DOLLAR, 'max' => 100 * M2_PER_DOLLAR],
    'AIRBNB' => ['min' => 25 * M2_PER_DOLLAR, 'max' => 60 * M2_PER_DOLLAR],
    'GOOGLE_PLAY' => ['min' => 5 * M2_PER_DOLLAR, 'max' => 200 * M2_PER_DOLLAR],
    'GROUPON' => ['min' => 10 * M2_PER_DOLLAR, 'max' => 200 * M2_PER_DOLLAR],
    'STARBUCKS' => ['min' => 5 * M2_PER_DOLLAR, 'max' => 100 * M2_PER_DOLLAR],
    'ROBLOX' => ['min' => 10 * M2_PER_DOLLAR, 'max' => 100 * M2_PER_DOLLAR],
    'IKEA' => ['min' => 25 * M2_PER_DOLLAR, 'max' => 1000 * M2_PER_DOLLAR]
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['redeem_submit'])) {
    ...
    } elseif ($amount > $m2_balance) {
        $redeemMessage = 'Insufficient balance. Available: ' . number_format($m2_balance, 2) . ' m²';
    } else {
        try {
            // Calculate fees based on method (all in m² tokens)
            switch ($method) {
                case 'faucetpay':
                    if ($amount < (0.02 * M2_PER_DOLLAR)) {
                        $redeemMessage = 'Minimum amount for FaucetPay is ' . number_format(0.02 * M2_PER_DOLLAR) . ' m²';
                        break;
                    }
                    $fee = 0;
                    $netAmount = $amount;
                ...
                case 'direct_wallet':
                    if ($amount < (5 * M2_PER_DOLLAR)) {
                        $redeemMessage = 'Minimum amount for Direct Wallet is ' . number_format(5 * M2_PER_DOLLAR) . ' m²';
                        break;
                    }
                    $fee = (0.05 * M2_PER_DOLLAR) + ($amount * 0.006); // 50,000 m² + 0.6%
                    $netAmount = $amount - $fee;
                    $selectedMethod = 'Direct Wallet';
                    break;
                    
                case 'tonkeeper':
                    if ($amount < (2 * M2_PER_DOLLAR)) {
                        $redeemMessage = 'Minimum amount for Tonkeeper is ' . number_format(2 * M2_PER_DOLLAR) . ' m²';
                        break;
                    }
                    if ($coin !== 'TON') {
                    ...
                    $fee = 0.05 * M2_PER_DOLLAR; // 50,000 m²
                    $netAmount = $amount - $fee;
                    $selectedMethod = 'Tonkeeper';
                    break;
                    
                case 'bitcoin_email':
                    if ($amount < (15 * M2_PER_DOLLAR)) {
                        $redeemMessage = 'Minimum amount for Bitcoin by Email is ' . number_format(15 * M2_PER_DOLLAR) . ' m²';
                        break;
                    }
                    ...
                    $fee = (0.05 * M2_PER_DOLLAR) + ($amount * 0.001); // 50,000 m² + 0.1%
                    $netAmount = $amount - $fee;
                ...
                case 'gift_card':
                    ...
                    if ($amount < $cardInfo['min'] || $amount > $cardInfo['max']) {
                        $minFormatted = number_format($cardInfo['min']);
                        $maxFormatted = number_format($cardInfo['max']);
                        $redeemMessage = "Amount must be between " . $minFormatted . " m² and " . $maxFormatted . " m² for " . ucfirst(strtolower($giftCardType));
                        break;
                    }
                    
                    // Check if user is from US (simplified check)
                    $isUSUser = true; // Assuming all users can access for now
                    
                    if ($giftCardType === 'STEAM') {
                        // Steam special fee: 5% of amount
                        $fee = $amount * 0.05;
                    } else {
                        $fee = $isUSUser ? (0.10 * M2_PER_DOLLAR) : 0; // 100,000 m² for US users
                    }
            ...
            if (empty($redeemMessage) && $netAmount > 0) {
                ...
                $redeemData = [
                    ...
                    'amount_m2' => $amount, // Changed from amount_usd to amount_m2
                    'fee' => $fee,
                ...
                if ($redeemResponse['status'] === 201 || $redeemResponse['status'] === 200) {
                    ...
                    $newM2Balance = $m2_balance - $amount;
                    $updateData = [
                        'm2' => $newM2Balance,
...
<head>
<meta charset="UTF-8">
<title>Redeem m² Tokens - MineMechanics</title>
...
<div class="main-content">
  <div class="header">
    <div class="welcome-message">
      <h1>Redeem m² Tokens</h1>
      <p>Withdraw your m² earnings using various payment methods (1 m² = $1 value)</p>
    </div>
    <div>
  ...
  <div class="redeem-container">
    ...
    <div class="balance-display">
      <div class="balance-card m2">
        ...
        <div class="balance-value m2-balance"><?php echo number_format($m2_balance, 2); ?> m²</div>
      </div>
    ...
    <div class="redeem-methods">
      ...
      <div class="method-card" onclick="selectMethod('faucetpay')" id="faucetpay-card">
        ...
        <div class="method-min">
          ...
          <div class="method-min-value"><?php echo number_format(0.02 * M2_PER_DOLLAR); ?> m²</div>
        </div>
      </div>
      ...
      <div class="method-card" onclick="selectMethod('direct_wallet')" id="direct_wallet-card">
        ...
        <ul class="method-features">
          <li><i class="ph-check-circle"></i> 50,000 m² + 0.6% fee</li>
          <li><i class="ph-check-circle"></i> Wallet address required</li>
          <li><i class="ph-check-circle"></i> Wide coin selection</li>
        </ul>
        <div class="method-min">
          <div class="method-min-label">Minimum Amount</div>
          <div class="method-min-value"><?php echo number_format(5 * M2_PER_DOLLAR); ?> m²</div>
        </div>
      </div>
      ...
      <div class="method-card" onclick="selectMethod('tonkeeper')" id="tonkeeper-card">
        ...
        <ul class="method-features">
          <li><i class="ph-check-circle"></i> 50,000 m² flat fee</li>
          <li><i class="ph-check-circle"></i> TON address required</li>
          <li><i class="ph-check-circle"></i> No memo needed</li>
        </ul>
        <div class="method-min">
          <div class="method-min-label">Minimum Amount</div>
          <div class="method-min-value"><?php echo number_format(2 * M2_PER_DOLLAR); ?> m²</div>
        </div>
      </div>
      ...
      <div class="method-card" onclick="selectMethod('bitcoin_email')" id="bitcoin_email-card">
        ...
        <ul class="method-features">
          <li><i class="ph-check-circle"></i> 50,000 m² + 0.1% fee</li>
          <li><i class="ph-check-circle"></i> Proton Mail only</li>
          <li><i class="ph-check-circle"></i> Email must have receive feature enabled</li>
        </ul>
        <div class="method-min">
          <div class="method-min-label">Minimum Amount</div>
          <div class="method-min-value"><?php echo number_format(15 * M2_PER_DOLLAR); ?> m²</div>
        </div>
      </div>
      ...
      <div class="method-card" onclick="selectMethod('gift_card')" id="gift_card-card">
        ...
        <ul class="method-features">
          <li><i class="ph-check-circle"></i> 100,000 m² fee (except Steam)</li>
    ...
    <div class="redeem-form-section">
      <div class="form-header">
        ...
        <div>
          <h2 class="form-title">Redeem m² Tokens</h2>
      ...
      <form method="POST" action="" class="redeem-form" id="faucetpay-form">
        ...
        <div class="form-row">
          <div class="form-group">
            <label class="form-label">Amount to Redeem (m²)</label>
            <input type="number" name="amount" class="form-input" 
                   step="1" min="<?php echo 0.02 * M2_PER_DOLLAR; ?>" max="<?php echo $m2_balance; ?>"
                   placeholder="Enter amount in m²" required>
            <small style="color: var(--text-muted); display: block; margin-top: 5px;">
              Minimum: <?php echo number_format(0.02 * M2_PER_DOLLAR); ?> m² | Available: <?php echo number_format($m2_balance, 2); ?> m²
            </small>
          </div>
        ...
        <div class="fee-calculator">
          <div class="fee-row">
            ...
            <span class="fee-value" id="faucetpay-amount">0 m²</span>
          </div>
          <div class="fee-row">
            <span class="fee-label">Fee:</span>
            <span class="fee-value" id="faucetpay-fee">0 m²</span>
          </div>
          <div class="fee-row">
            <span class="fee-label">You Receive:</span>
            <span class="fee-value" id="faucetpay-net">0 m²</span>
          </div>
        </div>
      ...
      <form method="POST" action="" class="redeem-form" id="direct_wallet-form">
        ...
        <div class="form-row">
          <div class="form-group">
            <label class="form-label">Amount to Redeem (m²)</label>
            <input type="number" name="amount" class="form-input" 
                   step="1" min="<?php echo 5 * M2_PER_DOLLAR; ?>" max="<?php echo $m2_balance; ?>"
                   placeholder="Enter amount in m²" required>
            <small style="color: var(--text-muted); display: block; margin-top: 5px;">
              Minimum: <?php echo number_format(5 * M2_PER_DOLLAR); ?> m² | Available: <?php echo number_format($m2_balance, 2); ?> m²
            </small>
          </div>
        ...
        <div class="fee-calculator">
          <div class="fee-row">
            <span class="fee-label">Amount:</span>
            <span class="fee-value" id="direct_wallet-amount">0 m²</span>
          </div>
          <div class="fee-row">
            <span class="fee-label">Fee (50,000 m² + 0.6%):</span>
            <span class="fee-value" id="direct_wallet-fee">0 m²</span>
          </div>
          <div class="fee-row">
            <span class="fee-label">You Receive:</span>
            <span class="fee-value" id="direct_wallet-net">0 m²</span>
          </div>
        </div>
      ...
      <form method="POST" action="" class="redeem-form" id="tonkeeper-form">
        ...
        <div class="form-row">
          <div class="form-group">
            <label class="form-label">Amount to Redeem (m²)</label>
            <input type="number" name="amount" class="form-input" 
                   step="1" min="<?php echo 2 * M2_PER_DOLLAR; ?>" max="<?php echo $m2_balance; ?>"
                   placeholder="Enter amount in m²" required>
            <small style="color: var(--text-muted); display: block; margin-top: 5px;">
              Minimum: <?php echo number_format(2 * M2_PER_DOLLAR); ?> m² | Available: <?php echo number_format($m2_balance, 2); ?> m²
            </small>
          </div>
        ...
        <div class="fee-calculator">
          <div class="fee-row">
            <span class="fee-label">Amount:</span>
            <span class="fee-value" id="tonkeeper-amount">0 m²</span>
          </div>
          <div class="fee-row">
            <span class="fee-label">Fee:</span>
            <span class="fee-value" id="tonkeeper-fee">50,000 m²</span>
          </div>
          <div class="fee-row">
            <span class="fee-label">You Receive:</span>
            <span class="fee-value" id="tonkeeper-net">0 m²</span>
          </div>
        </div>
      ...
      <form method="POST" action="" class="redeem-form" id="bitcoin_email-form">
        ...
        <div class="form-row">
          <div class="form-group">
            <label class="form-label">Amount to Redeem (m²)</label>
            <input type="number" name="amount" class="form-input" 
                   step="1" min="<?php echo 15 * M2_PER_DOLLAR; ?>" max="<?php echo $m2_balance; ?>"
                   placeholder="Enter amount in m²" required>
            <small style="color: var(--text-muted); display: block; margin-top: 5px;">
              Minimum: <?php echo number_format(15 * M2_PER_DOLLAR); ?> m² | Available: <?php echo number_format($m2_balance, 2); ?> m²
            </small>
          </div>
        ...
        <div class="fee-calculator">
          <div class="fee-row">
            <span class="fee-label">Amount:</span>
            <span class="fee-value" id="bitcoin_email-amount">0 m²</span>
          </div>
          <div class="fee-row">
            <span class="fee-label">Fee (50,000 m² + 0.1%):</span>
            <span class="fee-value" id="bitcoin_email-fee">0 m²</span>
          </div>
          <div class="fee-row">
            <span class="fee-label">You Receive:</span>
            <span class="fee-value" id="bitcoin_email-net">0 m²</span>
          </div>
        </div>
      ...
      <form method="POST" action="" class="redeem-form" id="gift_card-form">
        ...
        <div class="form-row">
          <div class="form-group">
            <label class="form-label">Amount to Redeem (m²)</label>
            <input type="number" name="amount" class="form-input" 
                   id="gift_amount" step="1" min="5" max="<?php echo $m2_balance; ?>"
                   placeholder="Enter amount in m²" required>
            <small style="color: var(--text-muted); display: block; margin-top: 5px;">
              Minimum: <?php echo number_format(5 * M2_PER_DOLLAR); ?> m² | Available: <?php echo number_format($m2_balance, 2); ?> m²
            </small>
            ...
            <select name="gift_card_type" class="form-select" id="gift_card_type" required>
              ...
              <?php foreach ($giftCards as $type => $info): ?>
                <option value="<?php echo $type; ?>" 
                  ...
                  <?php echo ucwords(strtolower(str_replace('_', ' ', $type))); ?> (<?php echo number_format($info['min']); ?> - <?php echo number_format($info['max']); ?> m²)
                </option>
              <?php endforeach; ?>
            </select>
          </div>
        ...
        <div class="fee-calculator">
          <div class="fee-row">
            <span class="fee-label">Amount:</span>
            <span class="fee-value" id="gift_card-amount">0 m²</span>
          </div>
          <div class="fee-row">
            <span class="fee-label">Fee:</span>
            <span class="fee-value" id="gift_card-fee">0 m²</span>
          </div>
          <div class="fee-row">
            <span class="fee-label">You Receive:</span>
            <span class="fee-value" id="gift_card-net">0 m²</span>
          </div>
        </div>
          ...
          <tbody>
            <?php foreach ($redeemHistory as $redeem): ?>
              <tr>
                ...
                <td class="amount-cell"><?php echo number_format($redeem['amount_m2'] ?? $redeem['amount_usd'], 2); ?> m²</td>
                <td class="amount-cell"><?php echo number_format($redeem['fee'], 2); ?> m²</td>
                <td class="amount-cell"><?php echo number_format($redeem['net_amount'] ?? (($redeem['amount_m2'] ?? $redeem['amount_usd']) - $redeem['fee']), 2); ?> m²</td>
...
<script>
// Constants
const M2_PER_DOLLAR = <?php echo M2_PER_DOLLAR; ?>;
const MIN_FAUCETPAY = <?php echo 0.02 * M2_PER_DOLLAR; ?>;
const MIN_DIRECT_WALLET = <?php echo 5 * M2_PER_DOLLAR; ?>;
const MIN_TONKEEPER = <?php echo 2 * M2_PER_DOLLAR; ?>;
const MIN_BITCOIN_EMAIL = <?php echo 15 * M2_PER_DOLLAR; ?>;
const MIN_GIFT_CARD = <?php echo 5 * M2_PER_DOLLAR; ?>;
const USER_M2_BALANCE = <?php echo $m2_balance; ?>;
...
// Fee calculation functions (all in m² tokens)
function calculateFaucetPayFee(amount) {
    return 0; // No fee
}

function calculateDirectWalletFee(amount) {
    return (0.05 * M2_PER_DOLLAR) + (amount * 0.006); // 50,000 m² + 0.6%
}

function calculateTonkeeperFee(amount) {
    return 0.05 * M2_PER_DOLLAR; // 50,000 m²
}

function calculateBitcoinEmailFee(amount) {
    return (0.05 * M2_PER_DOLLAR) + (amount * 0.001); // 50,000 m² + 0.1%
}

function calculateGiftCardFee(amount, cardType) {
    if (cardType === 'STEAM') {
        return amount * 0.05; // 5% of amount for Steam
    }
    return 0.10 * M2_PER_DOLLAR; // 100,000 m² for other gift cards
}

// Format m² amount with commas
function formatM2(amount) {
...
function updateFeeCalculator(method) {
    ...
    // Update display with formatted m² amounts
    document.getElementById(method + '-amount').textContent = formatM2(amount);
    document.getElementById(method + '-fee').textContent = formatM2(fee);
    document.getElementById(method + '-net').textContent = formatM2(netAmount);
}

// Gift card amount validation
document.getElementById('gift_card_type')?.addEventListener('change', function() {
    const selectedOption = this.options[this.selectedIndex];
    const min = parseFloat(selectedOption.getAttribute('data-min')) || MIN_GIFT_CARD;
    const max = parseFloat(selectedOption.getAttribute('data-max')) || USER_M2_BALANCE;
    
    const amountInput = document.getElementById('gift_amount');
    amountInput.min = min;
    amountInput.max = Math.min(max, USER_M2_BALANCE);
    
    // Update placeholder and validation message
    amountInput.placeholder = `Enter amount (${formatM2(min)} - ${formatM2(Math.min(max, USER_M2_BALANCE))})`;
    
    const small = amountInput.nextElementSibling;
    if (small) {
        small.textContent = `Minimum: ${formatM2(min)} | Maximum: ${formatM2(Math.min(max, USER_M2_BALANCE))} | Available: ${formatM2(USER_M2_BALANCE)}`;
    }
